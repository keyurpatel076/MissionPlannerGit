...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        public Button ButtonSurvey { get { return surveyGrid; } } //MB_270414
        ...
        private void Planner_Load(object sender, EventArgs e)
        {
            ...
            panelWaypoints.Expand = true;  //MB_20140419
        ...
        private void ChangeColumnHeader(string command)
        {
            ...
            cellcmd = Commands.Rows[selectedrow].Cells[ShowCommand.Index] as DataGridViewComboBoxCell;

            cellcmd.Items.Clear();
            if ((command != MAVLink.MAV_CMD.WAYPOINT.ToString())
             && (command != MAVLink.MAV_CMD.LOITER_UNLIM.ToString() )
             && (command != MAVLink.MAV_CMD.LOITER_TURNS.ToString())
             && (command != MAVLink.MAV_CMD.LOITER_TIME.ToString())
             && (command != MAVLink.MAV_CMD.LAND.ToString())
             && (command != MAVLink.MAV_CMD.TAKEOFF.ToString())
             && (command != MAVLink.MAV_CMD.DO_SET_CAM_TRIGG_DIST.ToString())
             && (command != MAVLink.MAV_CMD.DO_CHANGE_SPEED.ToString())) cellcmd.Items.Add(command);

            cellcmd.Items.Add(MAVLink.MAV_CMD.WAYPOINT.ToString());
            cellcmd.Items.Add(MAVLink.MAV_CMD.LOITER_UNLIM.ToString());
            cellcmd.Items.Add(MAVLink.MAV_CMD.LOITER_TURNS.ToString());
            cellcmd.Items.Add(MAVLink.MAV_CMD.LOITER_TIME.ToString());
            cellcmd.Items.Add(MAVLink.MAV_CMD.LAND.ToString());
            cellcmd.Items.Add(MAVLink.MAV_CMD.TAKEOFF.ToString());
            cellcmd.Items.Add(MAVLink.MAV_CMD.DO_SET_CAM_TRIGG_DIST.ToString());
            cellcmd.Items.Add(MAVLink.MAV_CMD.DO_CHANGE_SPEED.ToString());

            cellcmd.Value = command;
        ...
        private void Commands_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
            ...
            DataGridViewComboBoxCell showcell = Commands.Rows[e.RowIndex].Cells[ShowCommand.Index] as DataGridViewComboBoxCell; //MB_20140511
            showcell.DropDownWidth = 200; //MB_20140511
        ...
        void processToScreen(List<Locationwp> cmds, bool append = false)
        {
            ...
            foreach (Locationwp temp in cmds)
            {
                ...
                DataGridViewTextBoxCell cell;
                DataGridViewComboBoxCell cellcmd;
                DataGridViewComboBoxCell cellshowcmd; //MB_20140511
                cellcmd = Commands.Rows[i].Cells[Command.Index] as DataGridViewComboBoxCell;
                cellshowcmd = Commands.Rows[i].Cells[ShowCommand.Index] as DataGridViewComboBoxCell; //MB_20140511
                ...
                foreach (object value in Enum.GetValues(typeof(MAVLink.MAV_CMD)))
                {
                    if ((int)value == temp.id)
                    {
                        ...
                        //MB_20140511
                        cellshowcmd.Items.Clear();
                        if ((value.ToString() != MAVLink.MAV_CMD.WAYPOINT.ToString())
                         && (value.ToString() != MAVLink.MAV_CMD.LOITER_UNLIM.ToString())
                         && (value.ToString() != MAVLink.MAV_CMD.LOITER_TURNS.ToString())
                         && (value.ToString() != MAVLink.MAV_CMD.LOITER_TIME.ToString())
                         && (value.ToString() != MAVLink.MAV_CMD.LAND.ToString())
                         && (value.ToString() != MAVLink.MAV_CMD.TAKEOFF.ToString())
                         && (value.ToString() != MAVLink.MAV_CMD.DO_SET_CAM_TRIGG_DIST.ToString())
                         && (value.ToString() != MAVLink.MAV_CMD.DO_CHANGE_SPEED.ToString())) cellshowcmd.Items.Add(value.ToString());

                        cellshowcmd.Items.Add(MAVLink.MAV_CMD.WAYPOINT.ToString());
                        cellshowcmd.Items.Add(MAVLink.MAV_CMD.LOITER_UNLIM.ToString());
                        cellshowcmd.Items.Add(MAVLink.MAV_CMD.LOITER_TURNS.ToString());
                        cellshowcmd.Items.Add(MAVLink.MAV_CMD.LOITER_TIME.ToString());
                        cellshowcmd.Items.Add(MAVLink.MAV_CMD.LAND.ToString());
                        cellshowcmd.Items.Add(MAVLink.MAV_CMD.TAKEOFF.ToString());
                        cellshowcmd.Items.Add(MAVLink.MAV_CMD.DO_SET_CAM_TRIGG_DIST.ToString());
                        cellshowcmd.Items.Add(MAVLink.MAV_CMD.DO_CHANGE_SPEED.ToString());

                        cellshowcmd.Value = value.ToString();
                        // MB_____
        ...
        */
        ...
        void Commands_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            //MB_20140511
            Commands.Rows[selectedrow].Cells[ShowCommand.Index].Value = ((ComboBox)sender).Text;
        ...
        private void TXT_homelng_Enter(object sender, EventArgs e)  //MB_20140518
        {
            sethome = true;
            CustomMessageBox.Show("Click on the Map to set Home ");
        }
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        private void elevGraph_Click(object sender, EventArgs e)
        {
            writeKML();
            double homealt;
            double.TryParse(TXT_homealt.Text, out homealt);
